            ...
            if (is_array($data) && count($data) > 0) {
                ...
                for ($i = 0, $len = count($data); $i < $len; $i++) {
                    ...
                    if (isset($data[$i]->name, $data[$i]->description, $data[$i]->active)) {
                        ...
                        $code = $sql->errorCode();
                        if ($code == '00000') {
                            $id = $this->os->db->conn->lastInsertId('qo_groups_id_seq');
                        } else {
                    ...
                    if ($id) {
                        ...
                        $tipoaudit = "Administración grupos (Nuevo)";
                        $this->os->auditoria($tipoaudit, "INSERT; TABLA qo_groups; ID:" . $id . "; VALORES: " . $cadena);
                        /*fin auditoria*/
                    } else {
            ...
            if (is_array($data) && count($data) > 0) {
                ...
                for ($i = 0, $len = count($data); $i < $len; $i++) {
                    ...
                    if ($code == '00000') {
                        ...
                        $tipoaudit = "Administración grupos (Actualizar)";
                        $this->os->auditoria($tipoaudit, "UPDATE; TABLA qo_groups; ID:" . $data[$i]->id . "; VALORES: " . $temp);
                    } else {
        ...
        if (is_array($data) && count($data) > 0) {
            for ($i = 0, $len = count($data); $i < $len; $i++) {
                ...
                if ($success) {
                    ...
                    $tipoaudit = "Administración grupos (Eliminar)";
                    $this->os->auditoria($tipoaudit, "DELETE; TABLA qo_groups; ID:" . $data[$i]->id);
