            ...
            if (is_array($data) && count($data) > 0) {
                ...
                for ($i = 0, $len = count($data); $i < $len; $i++) {
                    ...
                    if (isset($data[$i]->name, $data[$i]->description, $data[$i]->active)) {
                        ...
                        $data[$i]->active = ($data[$i]->active == FALSE) ? 0 : $data[$i]->active;
                        $sql->bindParam(3, $data[$i]->active);
                        $sql->execute();

                        $code = $sql->errorCode();
                        if ($code == '00000') {
                            $id = $this->os->db->conn->lastInsertId('qo_privileges_id_seq');
                        } else {
                    ...
                    if ($id) {
                        ...
                        /*auditoria*/
                        $cadena = $data[$i]->name;
                        $cadena = $cadena . ', ' . $data[$i]->description;

                        $tipoaudit = "Administración privilegios (Nuevo)";
                        $this->os->auditoria($tipoaudit, "INSERT; TABLA qo_privileges; ID:" . $id . "; VALORES: " . $cadena);
                        /*fin auditoria*/
            ...
            if (is_array($data) && count($data) > 0) {
                ...
                for ($i = 0, $len = count($data); $i < $len; $i++) {
                    ...
                    if ($code == '00000') {
                        ...
                        $tipoaudit = "Administración privilegios (Actualizar)";
                        $this->os->auditoria($tipoaudit, "UPDATE; TABLA qo_privileges; ID:" . $data[$i]->id . "; VALORES: " . $temp);
                        /*fin auditoria*/
        ...
        if (is_array($data) && count($data) > 0) {
            for ($i = 0, $len = count($data); $i < $len; $i++) {
                ...
                if ($success) {
                    ...
                    /*auditoria*/
                    $tipoaudit = "Administración privilegios (Eliminar)";
                    $this->os->auditoria($tipoaudit, "DELETE; TABLA qo_privileges; ID:" . $data[$i]->id);
                    /*fin auditoria*/
