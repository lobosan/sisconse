...
class QoAdminMember
{
    ...
    public function view()
    {
        ...
        $filter_text = isset($_POST['filterText']) ? trim($_POST['filterText']) : NULL;
        // paging
    ...
    public function loadDCZ()
    {
        ...
        $sql = "SELECT
                ...
                (
                    ...
                    FROM
                        fic_dcz b
                    WHERE
                        b.id_dcz = SUBSTRING (A .id_dcz FROM 1 FOR 2)
                ) || ' - ' || A .nombre_dcz nombre_dcz
            FROM
                fic_dcz A
            WHERE
                CHAR_LENGTH (A .id_dcz) = 4
            ORDER BY
                id_dcz";
        ...
        if ($result) {
            ...
            $response = '{"fic_dcz":' . json_encode($items) . '}';
        }
        print $response;
    } //end loadDCZ()
            ...
            if (is_array($data) && count($data) > 0) {
                ...
                for ($i = 0, $len = count($data); $i < $len; $i++) {
                    ...
                    if (isset($data[$i]->first_name, $data[$i]->last_name, $data[$i]->email_address, $data[$i]->password, $data[$i]->locale, $data[$i]->active)) {
                        ...
                        $data[$i]->active = ($data[$i]->active == '') ? 0 : $data[$i]->active;
                        $sql->bindParam(6, $data[$i]->active);
                        $sql->execute();

                        $code = $sql->errorCode();
                        if ($code == '00000') {
                            $id = $this->os->db->conn->lastInsertId('qo_members_id_seq');
                        } else {
                    ...
                    if ($id) {
                        ...
                        $result->created[] = $temp;
                        /*auditoria*/
                        $cadena = $data[$i]->last_name;
                        $cadena = $cadena . ', ' . $data[$i]->first_name;
                        $cadena = $cadena . ', ' . $data[$i]->email_address;

                        $tipoaudit = "Administración usuarios (Nuevo)";
                        $this->os->auditoria($tipoaudit, "INSERT; TABLA qo_member; ID:" . $id . "; VALORES: " . $cadena);
                        /*fin auditoria*/
                    } else {
                    ...
                    if ($code == '00000') {
                        ...
                        foreach ($data[$i] AS $key => $value) {
                            if ($key !== 'id') {
                                ...
                                $temp .= $temp !== '' ? ', ' . $key . ' = ' . $valor : $key . ' = ' . $valor;
                            }
                        }
                        $tipoaudit = "Administración usuarios (Actualizar)";
                        $this->os->auditoria($tipoaudit, "UPDATE; TABLA qo_members; ID:" . $data[$i]->id . "; VALORES: " . $temp);
                        /*fin auditoria*/
        ...
        if (is_array($data) && count($data) > 0) {
            for ($i = 0, $len = count($data); $i < $len; $i++) {
                ...
                if ($success) {
                    ...
                    $tipoaudit = "Administración usuarios (Eliminar)";
                    $this->os->auditoria($tipoaudit, "DELETE; TABLA qo_member; ID:" . $data[$i]->id);
                } else {
    ...
    public function edit_groups()
    {
        ...
        for ($i = 0, $len = count($add); $i < $len; $i++) {
            if ($this->add_to_group($member_id, $add[$i], true, 0) == false) {
                $error = true;
            }
        }
        // delete
