...
QoDesk.QoAdmin.Privileges = Ext.extend(Ext.grid.EditorGridPanel, {
   ...
   , constructor : function(config){
      ...
      var searchFieldBtn = new Ext.Button({
         menu: new Ext.menu.Menu({
            items: [
               { checked: true, checkHandler: checkHandler, group: 'filterField', key: 'name', scope: this, text: 'Privilegio' }
               , { checked: false, checkHandler: checkHandler, group: 'filterField', key: 'description', scope: this, text: 'Descripción' }
               //, { checked: false, checkHandler: checkHandler, group: 'filterField', key: 'active', scope: this, text: 'Activo?' }
            ]
         })
         , text: 'Privilegio'
      });
	   ...
	   Ext.applyIf(config, {
         ...
         , bbar: new Ext.PagingToolbar({
            displayInfo: true
            , displayMsg: 'Mostrando privilegios {0} - {1} de {2}'
            , emptyMsg: 'No hay privilegios que mostrar'
            , pageSize: 30
            , store: store
         })
         , closable: true
         ...
         , tbar: [
            {
               ...
               , scope: this
               , text: 'Añadir'
               , tooltip: 'Añadir nuevos privilegios'
            }
            , '-'
            , {
               disabled: true
               ...
               , scope: this
               , text: 'Eliminar'
               , tooltip: 'Eliminar privilegio seleccionado'
            }
            , '-'
            , {
               disabled: true
         ...
         , title: 'Privilegios'
	      , viewConfig: {
	         emptyText: 'No hay privilegios que mostrar...'
            //, ignoreAdd: true
   ...
   , deletePrivilege : function(record){
      Ext.MessageBox.confirm('Confirmación', 'Está seguro de eliminar el privilegio No.: ' + record.data.id + '?', function(btn){
         if(btn === "yes"){
            ...
            Ext.Ajax.request({
               callback: function(options, success, response){
                  ...
                  if(success){
                     ...
                     if(fCount > 0){
                        Ext.MessageBox.alert('Advertencia', fCount+' privilegio(s) no eliminado(s)!');
                     }
   ...
   , onPrivilegeClick : function(){
      ...
      this.gridEditor.show(record, function(data){
         this.showMask('Actualizando privilegios...');
         Ext.Ajax.request({
            callback: function(options, success, response){
               this.hideMask();
               if(success){
                  ...
                  if(decoded.success === true){
                     ...
                     Ext.MessageBox.alert('Advertencia', 'Error ocurrido en el servidor!');
                  }
               }else{
                  Ext.MessageBox.alert('Advertencia', 'Conexión perdida con el servidor!');
               }
         ...
         callback: function(options, success, response){
            ...
            if(success){
               ...
               if(failed.length > 0){
                  ...
                  Ext.MessageBox.alert('Advertencia', 'No todos los privilegios fueron creados!');
               }
               ...
               Ext.MessageBox.alert('Alerta', 'Asigne módulos al privilegio creado!');
            }else{
               Ext.MessageBox.alert('Advertencia', 'Conexión perdida con el servidor!');
            }
         }
