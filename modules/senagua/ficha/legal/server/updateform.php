...
$sql .= "UPDATE
		...
		SET
			...
			fic_proceso = :con_proceso,
			fic_fecha_autor_ini = :con_fecha_autoriz_ini,
			fic_fecha_autor_ult = :con_fecha_autoriz_fin,
			fic_tarifa_anual = :con_tarifa,
			fic_fecha_pago = :con_fecha_pago_ini,
			fic_fecha_haspag = :con_fecha_pago_fin,
			...
			cor_id_ini = :rio_autoriz_actual_id
		WHERE
		fic_id = :con_id;
		";
...
if ($_FILES["ruta"]["name"] != "") {
    ...
    if (!is_dir($ruta . $parroquia . '/' . $proceso))
        mkdir($ruta . $parroquia . '/' . $proceso, 0777);

    if (!is_dir($ruta . $parroquia . '/' . $proceso . '/' . $formulario))
        mkdir($ruta . $parroquia . '/' . $proceso . '/' . $formulario, 0777);

    if (!is_dir($ruta . $parroquia . '/' . $proceso . '/' . $formulario . '/thumbnails'))
        mkdir($ruta . $parroquia . '/' . $proceso . '/' . $formulario . '/thumbnails', 0777);

    $upload_dir = $ruta . $parroquia . '/' . $proceso . '/' . $formulario . '/';
...
valores_radio('estado_captacion');
/**152 **/
valores_radio('operacion_actual');
/**156**/
valores('anio_inicio', 160);
valores_radio('tenencia');
...
if ($upload) {
    ...
    if (esFichaNuevaLegal($ficha)):
        ...
        if (validacionFichaSelf($tipo, $formulario, $dpa, $proceso, $ficha)):
            ...
            setAuditoria("Usos legales (Actualizar)", $sql);
        else:
            if (!$validaFicha):
                $resul_migracion["errores"] = '{success: false, msg:"Ficha ya existe. No se grabarán los cambios"}';
                $resul_migracion["stop"] = 1;
